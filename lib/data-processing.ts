...
function calculateErrorRateForCondition(words: WordData[], condition: (word: WordData) => boolean): number {
...
function generateRecommendations(data: MonkeyTypeData): Recommendation[] {
  ...
  if (summary.avgErrorWordLength > summary.avgCorrectedWordLength) {
    patternItems.push(`You tend to make more errors on longer words (average length: ${summary.avgErrorWordLength.toFixed(1)} characters).`);
  } else if (summary.avgErrorWordLength < summary.avgCorrectedWordLength) {
    patternItems.push(`You interestingly make more errors on shorter words (average length: ${summary.avgErrorWordLength.toFixed(1)} characters).`);
  } else {
    patternItems.push(`Word length doesn't seem to significantly impact your error rate.`);
  }
  
  if (metrics.shortWordsErrorRate > metrics.overallErrorRate) {
    patternItems.push(`High error rate (${metrics.shortWordsErrorRate.toFixed(1)}%) for short words under 4 characters.`);
  }
  
  if (metrics.longWordsErrorRate > metrics.overallErrorRate) {
    patternItems.push(`High error rate (${metrics.longWordsErrorRate.toFixed(1)}%) for long words over 6 characters.`);
  }
  
  if (metrics.wordsWithSpecialCharsErrorRate > metrics.overallErrorRate) {
    patternItems.push(`High error rate (${metrics.wordsWithSpecialCharsErrorRate.toFixed(1)}%) for words with special characters.`);
  }
  ...
  if (errorWordFrequency.length > 0) {
    const topErrorWords = errorWordFrequency.slice(0, 5).map(w => w.word).join(', ');
    practiceItems.push(`Practice these challenging words: ${topErrorWords}`);
  }
  
  if (metrics.shortWordsErrorRate > metrics.overallErrorRate) {
    practiceItems.push(`Practice precision for short, common words which may receive less attention.`);
  }
  
  if (metrics.longWordsErrorRate > metrics.overallErrorRate) {
    practiceItems.push(`For longer words, try breaking them down into syllables in your mind as you type.`);
    practiceItems.push(`Practice typing exercises specifically focused on longer words.`);
  }
  
  if (metrics.wordsWithSpecialCharsErrorRate > metrics.overallErrorRate) {
    practiceItems.push(`Dedicate practice time to words with special characters or punctuation.`);
    practiceItems.push(`Work on finger positioning for reaching less common keys.`);
  }
...
export function processMonkeyTypeData(data: MonkeyTypeSession[]): MonkeyTypeData {
  ...
  for (const session of data) {
    ...
    for (const wordData of session.words) {
      ...
      allWords.push({
        ...
        isCapitalized: word.length > 0 && word[0] === word[0].toUpperCase() && word[0] !== word[0].toLowerCase()
      });
    }
  }
  
  // Create session summary
  const sessionIds = [...new Set(allWords.map(w => w.sessionId))];
  ...
  for (const sessionId of sessionIds) {
    ...
    const corrected = sessionWords.filter(w => w.reason === 'corrected').length;
  ...
  const errorLetterFrequency: LetterFrequency[] = Object.entries(errorLetterCounts)
    .map(([letter, count]) => ({ letter, count }))
    .sort((a, b) => b.count - a.count);
  
  const correctedLetterFrequency: LetterFrequency[] = Object.entries(correctedLetterCounts)
    .map(([letter, count]) => ({ letter, count }))
    .sort((a, b) => b.count - a.count);
  ...
  const metrics: ErrorMetrics = {
    overallErrorRate,
    shortWordsErrorRate: calculateErrorRateForCondition(allWords, w => w.wordLength < 4),
    mediumWordsErrorRate: calculateErrorRateForCondition(allWords, w => w.wordLength >= 4 && w.wordLength <= 6),
    longWordsErrorRate: calculateErrorRateForCondition(allWords, w => w.wordLength > 6),
    wordsWithSpecialCharsErrorRate: calculateErrorRateForCondition(allWords, w => w.hasSpecialChars),
    wordsWithNumbersErrorRate: calculateErrorRateForCondition(allWords, w => w.hasNumbers),
    uppercaseWordsErrorRate: calculateErrorRateForCondition(allWords, w => w.isUppercase),
    capitalizedWordsErrorRate: calculateErrorRateForCondition(allWords, w => w.isCapitalized)
  };
  
  // Calculate summary statistics
  const avgErrorWordLength = errorWords.length > 0 
    ? errorWords.reduce((sum, w) => sum + w.wordLength, 0) / errorWords.length 
    : 0;
  
  const avgCorrectedWordLength = correctedWords.length > 0 
    ? correctedWords.reduce((sum, w) => sum + w.wordLength, 0) / correctedWords.length 
    : 0;
  
  const mostCommonErrorWord = errorWordFrequency.length > 0 ? errorWordFrequency[0].word : 'N/A';
  
  const sessionWithHighestErrorRate = sessionSummary.length > 0 
    ? `Session ${sessionSummary.reduce((prev, current) => (prev.errorRate > current.errorRate) ? prev : current).sessionId}`
    : 'N/A';
  
  const sessionWithLowestErrorRate = sessionSummary.length > 0 
    ? `Session ${sessionSummary.reduce((prev, current) => (prev.errorRate < current.errorRate) ? prev : current).sessionId}`
    : 'N/A';
